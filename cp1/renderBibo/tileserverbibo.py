...
POSTGIS_TABLE = dict(
    ...
    password='12345',
    dbname='habibi',
    table='(select ST_MPolyFromText(ST_AsText(geom), 2) as geom, name from tamanwisata) as tamanwisata')
LAYER_NAME = 'tamanwisata'
...
def render_tile(layer, z, x, y):
    ...
    label = mapnik.TextSymbolizer(mapnik.Expression('[name]'), 'DejaVu Sans Book',12,mapnik.Color('yellow'
    ))
    label.halo_radius = 1
